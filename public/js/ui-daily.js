...
function renderAll(){
  ...
  document.getElementById('kpi-top').innerHTML=results.map(function(item,i){
    ...
    return '<div style="border:1px solid '+SEL_COLORS[i]+';border-radius:var(--rl);padding:.7rem .6rem;background:var(--bg2);">'
      ...
      +'<div class="kpi" style="padding:.4rem .1rem;"><div class="kv" style="color:var(--blue);font-size:1.2rem;">'+itNum(d.pueAvg,2)+'</div><div class="ku" style="font-size:1.0rem;"></div><div class="kl" style="font-size:1.0rem;">PUE</div></div>'
      +'</div></div>';
  }).join('');
  ...
  document.getElementById('kpi-side-content').innerHTML='<div style="display:grid;grid-template-columns:repeat('+nCols+',1fr);gap:1.2rem;">'+results.map(function(item,idx){
    ...
    var costoIT=Math.round(d.hours.reduce(function(s,r){return s+(p.dcBase*r.tariff);},0));
    ...
    var rows=[
      ...
      ['PUE medio',itNum(d.pueAvg,2),'','#58a6ff'],
    ...
    var rowsHtml=rows.map(function(row){
      var l=row[0],v=row[1],u=row[2],c=row[3];
      ...
      return '<div style="display:flex;justify-content:space-between;padding:2px 0;border-bottom:1px solid var(--border);'+(isIndented?'padding-left:10px;':'')+'"><span style="font-size:1.0rem;color:var(--text3);">'+l.trim()+'</span><span style="font-size:1.0rem;font-family:\'IBM Plex Mono\',monospace;color:'+c+';">'+v+'<span style="color:var(--text3);font-size:.95rem;margin-left:2px;">'+u+'</span></span></div>';
    }).join('');
